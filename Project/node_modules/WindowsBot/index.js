...
class WindowsBot{
    ...
    */
    ...
    /**初始化ocr服务
     * @param {string} ocrServerIp  ocr服务器IP，固定端口9527。当参数值为 "127.0.0.1"时 为本地DLL模式， 
     * 根据显卡驱动对应的版本，将 ocr_cuda11.8或者ocr_cuda10.1里面的所有文件，复制到WindowsDriver.exe同级目录下，不必打开ppocr.exe服务端
     * @param {{useAngleModel:boolean, enableGPU:boolean, enableTensorrt:boolean}} options 可选参数
     * useAngleModel 支持图像旋转。 默认false
     * enableGPU 启动GPU 模式。默认false
     * enableTensorrt 启动加速，仅 enableGPU = true 时有效，默认false 。图片太大可能会导致GPU内存不足，程序崩溃，谨慎使用
     * @return {Promise.<boolean>} 总是返回true
    */
    ...
    */
    ocrByHwnd = async (hwnd, left, top, right, bottom, thresholdType, thresh, maxval, mode = false) =>{
        let strData = this.setSendData("ocrByHwnd", hwnd, left, top, right, bottom, thresholdType, thresh, maxval, mode);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
    ...
    */
    async getWords(hwndOrFile, options = {}){
        let left = 0, top = 0, right = 0, bottom = 0;
        if(options["region"] != undefined){
            left = options["region"][0];
            top = options["region"][1];
        ...
        if(typeof(hwndOrFile) == 'number' || hwndOrFile.indexOf(".") == -1){//hwnd
            ...
            if(options["mode"] != undefined)
                mode = options["mode"];
            wordsResult = await this.ocrByHwnd(hwndOrFile, left, top, right, bottom, thresholdType, thresh, maxval, mode);
        }else//file
            wordsResult = await this.ocrByFile(hwndOrFile, left, top, right, bottom, thresholdType, thresh, maxval);
    ...
    */
    async findWords(hwndOrFile, words, options = {}){
        let left = 0, top = 0, right = 0, bottom = 0;
        if(options["region"] != undefined){
            left = options["region"][0];
            top = options["region"][1];
    ...
    /**初始yolo服务
     * @param {string} yoloServerIp  yolo服务器IP,端口固定9528
     * @param {string} modelPath  模型路径
     * @param {string} classesPath 种类路径，CPU模式需要此参数
     * @return {Promise.<boolean>} 成功返回 true，失败返回false
    */
     ...
     * @return {Promise.<[]>} 失败返回null，成功返回数组形式的识别结果
    */
    yoloByHwnd = async (hwnd, mode = false) =>{
        let strData = this.setSendData("yoloByHwnd", hwnd, mode);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == '' || strRet == "null" || strRet == "[]")
            return null;
     ...
     * @return {Promise.<[]>} 失败返回null，成功返回数组形式的识别结果
    */
    yoloByFile = async (imagePath) =>{
        let strData = this.setSendData("yoloByFile", imagePath);
        let byteRet = await this.sendData(strData);
        let strRet = byteRet.toString();
        if(strRet == '' || strRet == "null" || strRet == "[]")
            return null;
    ...
    */
    async yolo(hwndOrFile, mode = false){
        let result;
        if(typeof(hwndOrFile) == 'number' || hwndOrFile.indexOf(".") == -1){//hwnd
            result = await this.yoloByHwnd(hwndOrFile, mode);
        }else//file
            result = await this.yoloByFile(hwndOrFile);
...
//    /**激活 initSpeechService (不支持win7) ，数字人使用免费，弃用此函数
